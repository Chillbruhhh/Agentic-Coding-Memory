...
use crate::handlers::query::{GraphDirection, GraphQuery, TraversalAlgorithm};
...
impl GraphTraversalService {
    ...
    pub async fn execute_multi_hop(
        &self,
        query: &GraphQuery,
    ) -> Result<TraversalResult, GraphTraversalError> {
        ...
        match &query.algorithm {
            ...
            Some(TraversalAlgorithm::Path) => self.execute_path_traversal(query, max_depth).await,
            Some(TraversalAlgorithm::Shortest) => {
                self.execute_shortest_path(query, max_depth).await
            }
            None => Err(GraphTraversalError::InvalidQuery(
                "Algorithm not specified for multi-hop query".to_string(),
            )),
        }
    }
    ...
    async fn execute_collect_traversal(
        &self,
        query: &GraphQuery,
        max_depth: usize,
    ) -> Result<TraversalResult, GraphTraversalError> {
        ...
        while let Some((current_id, depth)) = queue.pop_front() {
            ...
            for relation in &relation_list {
                ...
                let query_result =
                    timeout(Duration::from_secs(5), self.db.client.query(query_str)).await;
    ...
    async fn execute_path_traversal(
        &self,
        query: &GraphQuery,
        max_depth: usize,
    ) -> Result<TraversalResult, GraphTraversalError> {
        ...
        for start_id in &query.start_nodes {
            ...
            let paths = self
                .find_paths_iterative(*start_id, query, max_depth)
                .await?;
        ...
        if !unique_nodes.is_empty() {
            let node_ids: Vec<String> = unique_nodes
                .iter()
    ...
    async fn find_paths_iterative(
        ...
        query: &GraphQuery,
        max_depth: usize,
    ) -> Result<Vec<Vec<Uuid>>, GraphTraversalError> {
        let mut all_paths = Vec::new();
                                        ...
                                        if let Some(outbound) = obj.get("outbound") {
                                            if let Some(arr) = outbound.as_array() {
                                                for node in arr {
                                                    ...
                                                    {
                                                        if let Ok(node_id) = Uuid::parse_str(
                                                            id_str.trim_start_matches("objects:"),
                                                        ) {
                                                            ids.push(node_id);
                                                        }
                                                    }
    ...
    async fn execute_shortest_path(
        ...
        let target_id = query.target_node.ok_or_else(|| {
            GraphTraversalError::InvalidQuery(
                "Target node required for shortest path algorithm".to_string(),
            )
        })?;

        for start_id in &query.start_nodes {
            if let Some(path) = self
                .find_shortest_path(*start_id, target_id, query, max_depth)
                .await?
            {
                // Fetch node objects for the path
                let node_ids: Vec<String> =
                    path.iter().map(|id| format!("objects:`{}`", id)).collect();
