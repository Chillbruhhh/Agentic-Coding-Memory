...
Skills are stored in `SKILLS/`. Load skills when relevant to your task.

### How to use skills

1. **Discover**: Read `SKILLS/README.md` to see available skills
2. **Load**: When a task involves AMP tools, read `SKILLS/amp-core/SKILL.md`
...
Load `SKILLS/amp-core/SKILL.md` when:
...
5. `references/workflows.md` - When following patterns
6. `references/examples.md` - When need concrete examples
7. `references/cache-policy.md` - When working with cache

**Task-specific guides:**
8. `references/artifact-guidelines.md` - When creating decisions/notes/changesets
9. `references/file-sync-guide.md` - When modifying files (provenance tracking)
10. `references/cache-guide.md` - When need cache tool mechanics


# Ignore ARCHON RELATED RULE IF ARCHONS NOT ACCESSIBLE
...
## Ensure to always reflect on our TASK.MD located in amp/task.md 
- reflect on the pending tasks
- execute on the remaining tasks when asked
- update the task list as you complete features

## Ensure were are always updating our DEVLOG.MD
- log any and all additions to the code base
- if we modify something ensure we explain it in the devlog and why we did