...
# 3. Search for specific knowledge (filters and graph_options are required, use {} for none)
amp_query(query: "authentication implementation", filters: {"type": ["symbol"]}, graph_options: {}, limit: 5)
...
# Search for authentication code (filters and graph_options are required, use {} for none)
...
AMP (Agentic Memory Protocol) is a vendor-neutral protocol for durable, unified memory in agentic software development. It provides persistent, shared knowledge for AI agents with hybrid retrieval (vector + graph + temporal), multi-language code parsing (10 languages), and real-time knowledge graph visualization.

## Build Commands

```bash
# Start all services with Docker (recommended)
cd amp && docker compose up
...
# CLI
cd amp/cli && cargo run
cd amp/cli && cargo build --release && cargo install --path .
...
cd amp/ui && npm run build && npm run tauri:build
```
...
# Integration tests (PowerShell - run with server active)
cd amp/scripts
.\test-crud.ps1           # Basic CRUD operations
.\test-query.ps1          # Query endpoint
.\test-embeddings.ps1     # Vector embeddings
.\test-relationships.ps1  # Graph relationships
.\test-hybrid-retrieval.ps1  # Combined search
.\test-codebase-parser.ps1   # Multi-language parsing
.\test-filelog.ps1        # File log generation
...
├─────────────────────────────────────┤
│    MCP Server (Rust + rmcp 0.13)    │  Port 8106
├─────────────────────────────────────┤
│    CLI (Ratatui)    │  Desktop UI   │  Port 8109
├─────────────────────────────────────┤
│    AMP Server (Rust + Axum 0.7)     │  Port 8105
├─────────────────────────────────────┤
│    SurrealDB 2.4 (embedded/remote)  │  Port 7505
│    - Document Store                 │
│    - Vector Index (MTREE, 1536 dim) │
│    - Graph Database                 │
└─────────────────────────────────────┘
```
...
- **amp/server/** - Core server: handlers/, services/, models/ (Axum + SurrealDB)
- **amp/cli/** - Terminal CLI with TUI (Ratatui)
- **amp/ui/** - React/Tauri desktop app with 3D graph visualization (Three.js)
- **amp/mcp-server/** - MCP integration for AI agents (13 tools)
- **amp/spec/** - OpenAPI spec and SurrealDB schema
- **amp/scripts/** - Test and deployment scripts (40+)
- **amp/SKILLS/** - Agent skill documentation (progressive disclosure pattern)
...
│   ├── objects.rs     # CRUD operations
│   ├── query.rs       # Hybrid search (text+vector+graph)
│   ├── artifacts.rs   # Decisions/changesets
│   ├── cache.rs       # Episodic memory
│   ├── codebase.rs    # File parsing (Tree-sitter)
│   └── leases.rs      # Multi-agent coordination
├── services/          # Business logic
│   ├── embedding.rs   # Vector generation (OpenAI/Ollama)
│   ├── graph.rs       # Traversal algorithms (BFS/Dijkstra)
│   ├── hybrid.rs      # Multi-modal search
│   └── codebase_parser.rs  # Tree-sitter integration
└── models/            # Data types (Symbol, Decision, ChangeSet, Run)
```

### Key Patterns

- **Arc<Database>** for shared database references across handlers
- **Async/await** throughout with Tokio runtime
- **5-second timeouts** on all DB operations
- **Hybrid retrieval**: Vector (40%) + Text (30%) + Graph (30%) weighted scoring
- **Pluggable embeddings**: OpenAI, Ollama, or none

## Configuration

Key environment variables (see `amp/server/.env.example`):

```bash
PORT=8105
DATABASE_URL=memory           # memory | file://amp.db | ws://host:7505
EMBEDDING_PROVIDER=none       # none | openai | ollama
OPENAI_API_KEY=sk-...
INDEX_PROVIDER=openai         # For AI file summaries
```

## Supported Languages (Tree-sitter)

Python, TypeScript, JavaScript, Rust, Go, C#, Java, C, C++, Ruby

## Key Documentation

- `amp/DEVLOG.md` - Development timeline and technical decisions
- `amp/TASKS.md` - Feature roadmap and completed work
- `amp/mcp-server/INTEGRATION.md` - Agent setup guide
- `amp/spec/openapi.yaml` - Complete API reference
